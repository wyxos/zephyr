...
describe('release shared helpers', () => {
    ...
    it('asks Codex for a suggested conventional commit message when available', async () => {
        ...
        expect(codexCall?.[1].at(-1)).toContain('Inspect the current repository and decide the best conventional commit message for the pending changes.')
        expect(codexCall?.[1].at(-1)).toContain('Use whatever read-only inspection you need.')
        expect(codexCall?.[1].at(-1)).toContain('Do not use scopes like "fix(scope): ...".')
        expect(codexCall?.[1].at(-1)).toContain('Reply with exactly one line in the format "<type>: <subject>".')
